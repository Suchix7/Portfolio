...
const Hero = () => {
  ...
  const wrapChars = (text) => {
    return text.split("").map((char, i) => (
      <span key={i} className="hero-char inline-block whitespace-pre will-change-transform">
        {char}
      </span>
    ));
  };
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-screen w-full flex flex-col bg-[#0a0a0a] text-white px-6 md:px-12 lg:px-20 overflow-hidden"
    >
      ...
      <footer className="w-full py-8 flex justify-between items-end opacity-20 pointer-events-none">
        <div className="space-y-1">
          <p className="text-[8px] font-mono tracking-widest uppercase">Kernel_Status: Active</p>
          <p className="text-[8px] font-mono tracking-widest uppercase">Render_Engine: 120fps</p>
        </div>
